/*
	...
	Modified by G.Zito to allow deregistration of delegates, like in events in c#
*/
...
namespace SA {
	...
	public:
		...
		multicast_delegate& operator -=(const delegate<RET(PARAMS...)>& another) {
			if (another.isNull()) return *this;
			std::remove( this->invocationList.begin(), this->invocationList.end(), another ) ;
			return *this;
		} //operator -=
		...
		void invoke_if( std::function<bool (void)>& p, PARAMS... arg) const {
			for (auto& item : invocationList) {
				if( p() ) break ;
				(*(item->stub))(item->object, arg...);
			}
